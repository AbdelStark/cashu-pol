...
pub struct PolService {
    epochs: Arc<RwLock<HashMap<u64, EpochState>>>,
...
impl PolService {
    pub fn new(epoch_duration_days: i64, max_epoch_history: usize) -> Self {
        Self {
    ...
    pub async fn rotate_epoch(&self) -> Result<u64, PolError> {
        ...
        *current_epoch = new_epoch_id;
        ...
        if epochs.len() > self.max_epoch_history {
            let mut epoch_ids: Vec<_> = epochs.keys().cloned().collect();
            epoch_ids.sort_unstable();

            while epochs.len() > self.max_epoch_history {
                if let Some(oldest_epoch) = epoch_ids.first() {
                    epochs.remove(oldest_epoch);
                }
                epoch_ids.remove(0);
            }
        }
    ...
    pub async fn generate_report(&self) -> Result<PolReport, PolError> {
        ...
        for (_, epoch_state) in epochs.iter() {
            ...
            let report = EpochReport {
                ...
                end_time: if epoch_state.epoch_id < *self.current_epoch.read().await {
    ...
    pub async fn verify_mint_proof(&self, epoch_id: u64, proof: &Proof) -> Result<bool, PolError> {
        ...
        if let Some(epoch_state) = epochs.get(&epoch_id) {
...
mod tests {
    ...
    async fn test_epoch_cleanup() {
        let max_history = 3;
        let service = PolService::new(30, max_history);
        service.initialize().await.unwrap();
